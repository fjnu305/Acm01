...
@Slf4j
@Component
public class LanqiaoContestMapper {

    private static final ZoneId ZONE_SHANGHAI = ZoneId.of("Asia/Shanghai");
    private static final String CONTEST_URL_TEMPLATE = "https://www.lanqiao.cn/contests/%s";
    private static final int FINISHED_KEEP_DAYS = 180;

    public List<ContestDTO> mapItems(List<LanqiaoContestItem> items) {
        ...
        for (LanqiaoContestItem item : items) {
            ContestDTO dto = mapItem(item, finishedCutoff);
            if (dto != null) {
                result.add(dto);
            }
        }
        log.info("[lanqiao] mapped {} of {} items", result.size(), items.size());
        return result;
    }

    private ContestDTO mapItem(LanqiaoContestItem item, LocalDateTime finishedCutoff) {

 ...
 * 爬虫 HTTP 工具类。
 * <p>
 * 失败时抛出 {@link CrawlFetchException}（含 http_status / total_attempts），
...
public class CrawlHttpClient {
    ...
    /** 记录最近一次 getBody 的实际 HTTP 尝试次数（含重试），供 CrawlPipeline 写日志 */
    ...
    /** 最近一次 {@link #getBody(String)} 的实际尝试次数，默认 1 */
    ...
    /**
     * GET 请求并返回响应体；重试耗尽时抛 {@link CrawlFetchException}（error_type=HTTP_ERROR/TIMEOUT）。
     */
    ...
    /** GET + JSON 反序列化；解析失败抛 PARSE_ERROR */

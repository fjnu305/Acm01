package org.fjnu305.acm01.module.contest.crawl.fetch.ccpc;
...
@Slf4j
@Component
public class CcpcContestMapper {

    private static final ZoneId ZONE_SHANGHAI = ZoneId.of("Asia/Shanghai");
    private static final String CONTEST_URL_TEMPLATE = "https://ccpc.io/contest/%s";
    private static final int FINISHED_KEEP_DAYS = 180;

    public List<ContestDTO> mapItems(List<CcpcContestItem> items) {
        ...
        for (CcpcContestItem item : items) {
            ContestDTO dto = mapItem(item, finishedCutoff);
            if (dto != null) {
                result.add(dto);
            }
        }
        log.info("[ccpc] mapped {} of {} items", result.size(), items.size());
        return result;
    }

    private ContestDTO mapItem(CcpcContestItem item, LocalDateTime finishedCutoff) {
        ...
        return ContestDTO.builder()
                ...
                .contestType("CCPC")
                .location(StringUtils.hasText(item.getLocation()) ? item.getLocation().trim() : "China")
                .build();
    }
